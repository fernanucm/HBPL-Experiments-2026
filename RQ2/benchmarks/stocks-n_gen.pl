% Generate the parametric benchmark stocks-n....pl
...
gen(Equations, Rules, Assumptions) :-
  generate_benchmark('stocks-n', Equations, Rules, Assumptions).
...
build_nested_string(K, N, Result) :-
    ...
    format(string(Result), "shareholder(paul, amazon_~d) => (stock_up(amazon_~d) with ~1f) => ~w", [K, K, Grade, Inner]).
