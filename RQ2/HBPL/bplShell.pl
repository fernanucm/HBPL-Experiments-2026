...
bpl_shell :-
	...
	(get_bpl_flag(timing(true))->
  	time(process_input(String, Arguments))
  ;
  	process_input(String, Arguments)
	),
...
%% process_input(+String, +Arguments)
%
%     Process given input string with arguments
process_input(String, Arguments) :-
	catch((
...
%% read_shell_line(+Prompt, ?String, ?Arguments, -Continue)
%
...
%     
%     Continue (yes or no) is intended to stop
...
pl_read_shell_line(Prompt, String, ArgumentsWithQuotes) :-
  ...
  % when closing the application in the Windows application
  (StringCodes == end_of_file
   ->
...
translate_command(String, Arguments, Command) :-
	% Checks if string contains a metacommand
	Arguments = [MetaCommand|CommandArguments],
	meta_command(MetaCommand),
	% Extracts command from string and parses it
	sub_atom(String, MetaCommandStart, _, _, MetaCommand),
	CommandStart is MetaCommandStart + 2,
	sub_atom(String, CommandStart, _, 0, CommandString),
	translate_command(CommandString, CommandArguments, CommandCommand),
	Command =.. [MetaCommand,CommandCommand],
	!.
...
translate_command(String, Arguments, Command) :-
	...
	translator:translate_query(QueryString, Query, NewHypoRules, Bindings, Degree),
	Command = sv(Query, NewHypoRules, Bindings, Degree),
	!.
...
translate_command(String, _Arguments, Command) :-
	...
	translator:translate_query(String, Query, NewHypoRules, Bindings, Degree),
	Command = sv(Query, NewHypoRules, Bindings, Degree),
	!.
...
%% load_file(+Filename, +OverwriteTPL, +ResetFlags, +CompileOnly)
%
...
%     Consulting the TPL file is avoided if CompileOnly is set to 'yes'.
%

%% load_file(+BPLFilename, +TPLFilename, +FullBPLFilename, +FullTPLFilename, +FullTPLSFilename, +OverwriteTPL, +ResetFlags, +CompileOnly)
%
%     Internal predicate called by load_file/4, which takes the full
...
load_file(Filename, OverwriteTPL, ResetFlags, CompileOnly) :-
	...
	load_file(BPLFilename, TPLFilename,
	          ...
	          OverwriteTPL, ResetFlags, CompileOnly),
  !.

load_file(BPLFilename, TPLFilename,
          FullBPLFilename, FullTPLFilename, FullTPLSFilename,
	      _OverwriteTPL, ResetFlags, CompileOnly) :-
	...
	load_bpl(BPLFilename, TPLFilename,
             FullBPLFilename, FullTPLFilename, FullTPLSFilename, 
             ResetFlags, CompileOnly).

load_file(BPLFilename, TPLFilename,
          FullBPLFilename, FullTPLFilename, FullTPLSFilename,
          yes, ResetFlags, CompileOnly) :-
	exists_file(FullBPLFilename),
...
load_file(BPLFilename, TPLFilename,
	...
	(utilities:file_is_newer(FullTPLFilename, FullBPLFilename) ->
		...
		(safe_tpl(FullTPLSFilename) ->
		  ...
		  writef_to_atom('\'%w\' already exists and is being loaded...\n', [TPLFilename], Message),
		  load_tpl(FullBPLFilename, FullTPLFilename, Message, ResetFlags, CompileOnly)
  	 ;
  		% TPL file is not safe and must be overwritten
  		delete_file(FullTPLFilename), 
		writef('\'%w\' must be recompiled.\n', [TPLFilename]),
  		load_bpl(BPLFilename, TPLFilename,
                   FullBPLFilename, FullTPLFilename, FullTPLSFilename, 
                   ResetFlags, CompileOnly)
		)
...
load_file(BPLFilename, TPLFilename,
	...
	writef_to_atom('WARNING: \'%w\' does not exist.\n\c
	                \'%w\' already exists and is being loaded...\n', 
				   [BPLFilename, TPLFilename], Message),
	load_tpl(FullBPLFilename, FullTPLFilename, Message, ResetFlags, no).

load_file(BPLFilename, _TPLFilename,
          FullBPLFilename, FullTPLFilename, _FullTPLSFilename, _OverwriteTPL, _ResetFlags, _CompileOnly) :-
	not(exists_file(FullBPLFilename)),
	not(exists_file(FullTPLFilename)),
...
load_bpl(BPLFilename, TPLFilename,
         FullBPLFilename, FullTPLFilename, FullTPLSFilename, ResetFlags, CompileOnly) :-
	...
	writef_to_atom('\'%w\' is being loaded...\n', [TPLFilename], Message),
	load_tpl(FullBPLFilename, FullTPLFilename, Message, ResetFlags, CompileOnly).

load_bpl(_BPLFilename, _TPLFilename,
         _FullBPLFilename, _FullTPLFilename, _FullTPLSFilename, _ResetFlags, _CompileOnly) :-
	% This is only executed if translate_program/4 fails
...
load_tpl(_FullBPLFilename, _FullTPLFilename, _Message, _ResetFlags, yes) :-
...
%% load_ontology(+Filename, +OverwriteTPL, +ResetFlags, +CompileOnly)
%
...
%% load_ontology(+BPLFilename, +ONTFilename, +TPLFilename, +FullBPLFilename, +FullONTFilename, +FullTPLFilename, +FullTPLSFilename, +OverwriteTPL, +ResetFlags, +CompileOnly)
%
...
load_ontology(_Filename, _OverwriteTPL, _ResetFlags, _CompileOnly) :-
...
load_ontology(BPLFilename, ONTFilename, TPLFilename,
	...
	load_bpl_with_ontology(BPLFilename, ONTFilename, TPLFilename,
                           FullBPLFilename, FullONTFilename, FullTPLFilename, FullTPLSFilename, ResetFlags, CompileOnly).
...
load_ontology(BPLFilename, ONTFilename, TPLFilename,
	...
	((utilities:file_is_newer(FullTPLFilename, FullBPLFilename),
	  utilities:file_is_newer(FullTPLFilename, FullONTFilename)) ->
		% TPL file is newer than ontology and BPL, so it is not
		% recompiled
		(safe_tpl(FullTPLSFilename) ->
		  % TPL file is safe to be loaded without recompiling (flags match)
...
load_ontology(_BPLFilename, ONTFilename, _TPLFilename,
	          ...
	          _OverwriteTPL, _ResetFlags, _CompileOnly) :-
	exists_file(FullBPLFilename),
	not(exists_file(FullONTFilename)),
	% Original BPL file exists but ontology doesn't
	writef('ERROR: \'%w\' ontology does not exist.\n', [ONTFilename]).
...
%% load_bpl_with_ontology(+BPLFilename, +ONTFilename, +TPLFilename, +FullBPLFilename, +FullONTFilename, +FullTPLFilename, +FullTPLSFilename, +ResetFlags, +CompileOnly)
%
...
load_bpl_with_ontology(_BPLFilename, _ONTFilename, _TPLFilename,
                       _FullBPLFilename, _FullONTFilename, _FullTPLFilename, _FullTPLSFilename, _ResetFlags, _CompileOnly) :-
...
%% sv(+Query, +NewHypoRules, +Bindings, ?Degree)
%
...
%
%     Auxiliary predicate used by sv/3.
%

sv(Query, NewHypoRules, Bindings, Degree) :-
  reset_first_answer_flag,
  assert_rules(NewHypoRules),
	catch(sv_aux(Query, Bindings, Degree), Exception, true),
	...
	!,
  % Remove hypothetical rules due to compile-time translations
  retract_rules(NewHypoRules),
  % Remove hypothetical rules due to higher order calls
  retract_hypo_rules,
  % Remove all flags of eval_negation
  flags:remove_all_bpl_flag(first_not_solution(_, _, _)),
  (nonvar(Exception) -> throw(Exception) ; true).


sv_aux(Query, [], Degree) :- 
	% Ground query (i.e., query has no free variables)
	evaluator:solve_goal(Query, Deterministic),
	retractall(first_answer_flag),
	write('true'),
	write_approximation_degree(Degree),
	sv_actions(Deterministic).
...
:- dynamic first_answer_flag/0.
reset_first_answer_flag :-
	retractall(first_answer_flag),
  assertz(first_answer_flag).
...
%% assert_rules(+Rules)
%
%   Asserts hypothetical rules in the list Rules, which have
%   the representation as given by translate_query
%

assert_rules(NewHypoRules) :-
  member(Rule, NewHypoRules),
  assertz(evaluator:Rule),
  fail.

assert_rules(_NewHypoRules).


%% retract_rules(+Rules)
%
%   Retracts hypothetical rules in the list Rules, which have
%   the representation as given by translate_query
%

retract_rules(NewHypoRules) :-
  member(Rule, NewHypoRules), 
  retract(evaluator:Rule),
  fail.

retract_rules(_NewHypoRules).


%% retract_hypo_rules
%
%   Retracts all hypothetical rules 
%
%   WARNING: Better register hypothetical additions to retract them
%

retract_hypo_rules :-
  flags:get_bpl_flag(hypothetical(true)),
  parser:program_prefix(Prefix),
  atom_concat(Prefix, '_', Prefix_),
  evaluator:reg(Id, _, _),
  (evaluator:current_predicate(Predicate/Arity),
   atom_length(Prefix_, Length),
   sub_atom(Predicate, 0, Length, _, Prefix_),
   functor(Head, Predicate, Arity),
   evaluator:clause(Head, Body),
   translator:actual_rule_arity(Arity, ActualArity),
   ActualArity1 is ActualArity + 1,
   arg(ActualArity1, Head, hypo(_, Id, CD, _)),
   CD \== [], % Do not remove rules defined by the user (i.e., pertaining to the top context)
   % Catch possible retracts of static procedures at evaluator
   catch(retract(evaluator:(Head :- Body)),
     error(permission_error(modify, static_procedure, _), _),
     true),
   fail
  ;
   retract(evaluator:reg(Id, _, _)),
   fail).
...
%     If Deterministic=true, the user is not asked.
%
...
%		writeln('Yes')
	...
	(member(Char, ['h', '?']) ->
		...
		writeln(' h (?): help'), nl,
		write('Action? '), sv_actions(Deterministic)
	;
		% Invalid action
		nl,
...
ts :-
  check_safe_execution,
  !,
  (exists_file('./test/bpl_fuzzysets.bpl') ->
  	writeln('Testing...'), nl,
  	test:main_test
  ; writeln('ERROR: This command must be issued from the directory where BPL is installed (hint: use the command ''cd Path'' to change to the directory Path).')).
ts.
...
%% tm(+Input)
%
%     Display the time needed for processing an input.
%
...
%% sp(+Predicate)
%
%     Set a spy point on a predicate.
%
...
st :-
  ...
  flags:get_bpl_flag(hypothetical(Hypothetical)),
  flags:get_bpl_flag(indexing(Indexing)),
  flags:get_bpl_flag(filtering(Filtering)),
  flags:get_bpl_flag(degree_accumulator(DegreeAccumulator)),
	write('Bousi~Prolog system status:\n'),
	format('   - Weak unification algorithm : ~w\n', [Algorithm]),
	format('   - Hypothetical mode          : ~w\n', [Hypothetical]),
	format('   - Indexing                   : ~w\n', [Indexing]),
	format('   - Filtering                  : ~w\n', [Filtering]),
	format('   - Degree accumulator         : ~w\n', [DegreeAccumulator]).
...
% 		number_chars(LambdaValue, LambdaChars),
		atom_number(Lambda, LambdaValue),
		LambdaValue >= 0, LambdaValue =< 1, 
		!,
		% Changes lambda-cut value
		flags:remove_bpl_flag(lambda_cut(OldLambdaValue)),
		flags:add_bpl_flag(lambda_cut(LambdaValue)),
		reload_on_lambda_change(OldLambdaValue, LambdaValue),
...
reload(Options) :-
	...
	ld(Program, [f | Options]), % f: force, r: reload (don't reset flags), c: compile only (do not consult the .tpl)
	%trace, notrace, % Workaround for bug with unexplainable behaviour, cf. todo.txt
	!.
	
% Or an ontology:
reload(Options) :-
	last_program_loaded(_Program, Ontology),
	ld(Ontology, [f, o | Options]), % f: force, o: ontology, r: reload (don't reset flags), c: compile only (do not consult the .tpl)
	!.
...
%     option is specified (recompiling). The 'c' option avoids loading
%     the TPL compiled file
%

ld(Filename, Options) :-
	is_list(Options),
	subset(Options, [f, o, r, c]),
	(member(f, Options) ->
		OverwriteTPL = yes
	;
		OverwriteTPL = no
...
%% ti(+Boolean)
%
%     Changes current timing behaviour: if Boolean=true, then
%     apply timing; otherwise, do not.
%
...
ti(Boolean) :-
	...
	!,
	% Changes filtering value
	flags:remove_bpl_flag(timing(_OldBoolean)),
	flags:add_bpl_flag(timing(Boolean)),
	writef('New timing is: %w\n', [Boolean]).
...
%% hy
%
%     Shows whether hypothetical mode is enabled.
%

hy :-
	flags:get_bpl_flag(hypothetical(Boolean)),
	writef('Hypothetical mode enabled: %w\n', [Boolean]).


%% hy(+Boolean)
%
%     Changes current hypothetical mode: if Boolean=true, then
%     it is set; otherwise, it is not.
%
...
hy(Boolean) :-
	...
	memberchk(Boolean, [true, false]),
	!,
	% Changes hypothetical value
	flags:remove_bpl_flag(hypothetical(OldBoolean)),
	flags:add_bpl_flag(hypothetical(Boolean)),
	directives:set_hypothetical_operators(Boolean),
	reload_on_flag_change(OldBoolean, Boolean),
	flags:get_bpl_flag(hypothetical(CurrentBoolean)),
	( % Flag changed due to directive
    Boolean == OldBoolean,
    OldBoolean \== CurrentBoolean,
    Text = 'Directive of loaded program changed hypothetical mode to'
	; % Flag changed due to command
    Boolean \== OldBoolean,
    OldBoolean \== CurrentBoolean,
    Text = 'New hypothetical mode is'
	; % Flag unchanged due to directive
    Boolean \== OldBoolean,
    OldBoolean == CurrentBoolean,
    Text = 'Unchanged hypothetical mode due to directive of loaded program'
	; % Flag unchanged due to command
    Boolean == OldBoolean,
    OldBoolean == CurrentBoolean,
    Text = 'Hypothetical mode remains as'
  ),
	writef('%w: %w\n', [Text, Boolean]).
...
command_arguments(sp, 1, no).
command_arguments(st, 0, no).
command_arguments(ti, 1, no).
command_arguments(hy, 1, no).


%% meta_command(?Command)
%
%     Classifies Command as a meta-command
%     For example, tm takes a shell input and displays its execution time
