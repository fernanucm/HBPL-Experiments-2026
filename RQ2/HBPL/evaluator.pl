...
:- use_module('../systems/bench_utils.pl').
...
solve_goal(Goal, Deterministic) :-
  evaluator:reset_di, % Reset hypothetical database context identifier
	retractall(evaluator:reg(_, _, _)), % Retract hypothetical registered rules
  	...
  	%time(Goal,RTime,CTime),
    Goal,
    stat_end(RT, T, I, G, L, H, RTime, CTime, Inferences, _Global, _Local, _Heap),
    retractall(bench_stats(_,_,_)),
    assertz(bench_stats(RTime,CTime,Inferences)),
