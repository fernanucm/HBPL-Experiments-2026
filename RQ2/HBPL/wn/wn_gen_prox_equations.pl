...
wn_auto_gen_prox_equations([:- directive(wn_gen_prox_equations, [Measure, Auto])], Rules, InEquations, OutEquations) :-
  \+ is_list(Auto),
  atoms_functors_in_term(Rules, AllTokenAtoms, AllTokenFunctors),
  token_atom_list(AllTokenAtoms, UnsortedAllAtoms),
  token_atom_list(AllTokenFunctors, UnsortedAllFunctors),
  exception_words(ExceptionWords),
  sort(UnsortedAllAtoms, AllAtoms),
  sort(UnsortedAllFunctors, AllFunctors),
...
% IMPORTANT: This must be an ordered set
exception_words([
  ':-',
...
%%% token_atom(+Token, -Atom)
%%%
%%%   Return the atom represented by Token. If Token does not represent an atom, then fail.
%%%   A constant token read by the tokenizer can be:
%%%      - An atom 'atom'.
%%%      - A quoted atom '\'quoted atom\''.
%%%      - A variable 'Variable'.
%%%      - A number, e.g., 1.
%%%

token_atom_list([], []).
token_atom_list([Token|TokenList], [Atom|AtomList]) :-
  token_atom(Token, Atom),
  !,
  token_atom_list(TokenList, AtomList).
token_atom_list([_Token|TokenList], AtomList) :-
  token_atom_list(TokenList, AtomList).

token_atom(Token, Atom) :-
  read_term_from_atom(Token,Atom,[]), 
  atom(Atom).
...
%%% The idea is that if the first parameter (the Mode parameter) is equal to "simple" the proximity equation "Equation"
