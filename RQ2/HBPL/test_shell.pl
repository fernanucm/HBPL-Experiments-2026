...
run_ld_tests :-
	...
	sleep(1), % Needed to wait OS for updating file modification
	tell(BPLProgramPath),
	write('b.'),
	told,
	run_command_test('ld program.bpl', ['ld', 'program.bpl'],
...
%  WARNING: This test fails in batch, but it does not in an interactive session:
% 	run_command_test('sv c', ['sv', 'c'],
  ...
  (test_auto(on) -> 
   ...
   ; true),
	...
	run_command_test('b(0).', ['b', '(', '0', ')', '.'],
	                 ['true', 'With approximation degree: 0.5']),
	...
	run_command_test('sv b(X)', ['sv', 'b', '(', 'X', ')'],
	                 ['X = 0', 'With approximation degree: 0.5 ;', 'X = 1', 'With approximation degree: 0.5.']),
  (test_auto(on) -> 
  	 close(Stream)
   ; true),
	% > Error cases
	...
	run_command_test('call(undefined(_))', ['call', '(', 'undefined', '(', '_', ')', ')'],
	                 ['bpl_call/5: Undefined procedure: undefined/1']),
%	                 ['catch/3: Undefined procedure: undefined/1']),
%	                 ['\'<meta-call>\'/1: Undefined procedure: undefined/1']),
	run_command_test('sv a(.', ['sv', 'a', '(', '.'],
...
%% test_auto(-Switch)
%
%     If Switch is on, the file shell_inputs is used to avoid to type inputs
...
run_hy_tests :-
	% > Empty command
	run_command_test('hy', ['hy'],
	                 ['Hypothetical mode enabled: false']),
	run_command_test('hy true', ['hy', 'true'],
	                 ['New hypothetical mode is: true']),
	run_command_test('hy false', ['hy', 'false'],
	                 ['New hypothetical mode is: false']).
