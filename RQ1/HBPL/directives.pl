...
:- module(directives, [
		exec_directive/4,       % +Name, +Arguments, +Program, -EquationsIn, -EquationsOut
...
% See some WordNet predicates as requested at run-time for parsing/processing directives
see_needed_wn_predicates :-
...
%     In addition, it may add new equations ruled by wn_gen_prox_equations
%

directive(Name, Arguments) :-
  exec_directive(Name, Arguments, _, _).


%% exec_directive(+Name, +Arguments, -EquationsIn, -EquationsOut)
%
%     This call is performed at compile-time. The last two
%     arguments is the difference list that contains the new 
%     equations that will replace the directive in the compiled 
%     program

exec_directive(lambda_cut, [Lambda]) -->
...
exec_directive(filtering, [Boolean]) -->
...
exec_directive(filtering_warning, [Boolean]) -->
	% Sets the filtering warning behavior, i.e., if Boolean=true, then
  % warn if an equation is filtered during parsing; otherwise, do not.
	{
  	flags:set_bpl_flag(filtering_warning(Boolean))
	},
	[].

exec_directive(indexing, [Boolean]) -->
	% Sets the indexing behavior, i.e., if Boolean=true, then
  % apply indexing; otherwise, do not.
	{
  	flags:set_bpl_flag(indexing(Boolean))
	},
	[].

exec_directive(degree_accumulator, [Boolean]) -->
	% Sets the degree_accumulator behavior, i.e., if Boolean=true, then
  % apply degree accumulator optimization; otherwise, do not.
	{
  	flags:set_bpl_flag(degree_accumulator(Boolean))
	},
	[].

exec_directive(ontology_warning, [Boolean]) -->
	% Sets the ontology warning behavior, i.e., if Boolean=true, then
  % warn if the loaded program does not contain rules; 
  % otherwise, do not.
	{
  	flags:set_bpl_flag(ontology_warning(Boolean))
	},
	[].

exec_directive(transitivity, [Type]) -->
...
exec_directive(fuzzy_rel, [RelName, ClosureProperties]) -->
...
exec_directive(domain, [Name, Min, Max, Unit]) -->
...
exec_directive(fuzzy_set, [DomainName, Subsets]) -->
...
exec_directive(weak_unification, [Type]) -->
...
exec_directive(ext_block_equs, [Boolean]) -->
...
exec_directive(fuzzy_logic, [TNorm]) -->
...
exec_directive(wn_connect, []) -->
  % Loads WordNet and wn_connect
	{
    evaluator:ensure_loaded_wn,
  	flags:set_bpl_flag(wordnet_loaded(true))
	},
	[].

exec_directive(wn_connect, [QFolder]) -->
...
exec_directive(wn_gen_prox_equations, [Measure, ListOfListOfWords]) -->
...
exec_directive(wn_gen_prox_equations, [_Measure, _Auto]) -->
...
exec_directive(hypothetical, [Boolean]) -->
	% Sets the filtering behavior, i.e., if Boolean=true, then
  % set hypothetical mode; otherwise, disable it.
	{
  	flags:set_bpl_flag(hypothetical(Boolean)),
  	set_hypothetical_operators(Boolean)
	},
	[].


%% declare_hypothetical_operators/0
%
%     Declare '=>' and '/\\' as the embedded implication and hypothetical 
%     conjunction operators

declare_hypothetical_operators :-
  evaluator:op(1050, xfy, [=>]),      % Declare hypothetical implication operator
  evaluator:op(1020, xfy, [/\]).     % Declare hypothetical conjunction operator

%% remove_hypothetical_operators/0
%
%     Remove the declarations of the operators '=>' and '/\\' (embedded 
%     implication and hypothetical conjunction)

remove_hypothetical_operators :-
  evaluator:op(0,xfy,[=>]),         % Remove hypothetical implication operator
  evaluator:op(0,xfy,[/\]).        % Remove hypothetical conjunction operator


%% set_hypothetical_operators(+Boolean)
%
%     If Boolean == true, declare '=>' and '/\\' as the embedded implication
%     and hypothetical conjunction operators; else, remove the declarations
...
set_hypothetical_operators :-
	flags:get_bpl_flag(hypothetical(Hypothetical)),
	set_hypothetical_operators(Hypothetical).
...
is_directive_valid(filtering_warning, [Boolean]) :-
	memberchk(Boolean,[true,false]),
	% Directive is OK
	!.

is_directive_valid(filtering_warning, [_Boolean]) :-
	% Filtering warning value is invalid
	!,
	throw_invalid_argument_error(filtering_warning, 1,
		'Filtering warning value must be either \'true\' or \'false\'.').

is_directive_valid(filtering_warning, Arguments) :-
	% Wrong number of arguments
	length(Arguments, Arity),
	throw_wrong_arity_error(filtering_warning, Arity, 1).

%
% :- indexing(Boolean)
%

is_directive_valid(indexing, [Boolean]) :-
	memberchk(Boolean,[true,false]),
	% Directive is OK
	!.

is_directive_valid(indexing, [_Boolean]) :-
	% indexing value is invalid
	!,
	throw_invalid_argument_error(indexing, 1,
		'indexing value must be either \'true\' or \'false\'.').

is_directive_valid(indexing, Arguments) :-
	% Wrong number of arguments
	length(Arguments, Arity),
	throw_wrong_arity_error(indexing, Arity, 1).

%
% :- degree_accumulator(Boolean)
%

is_directive_valid(degree_accumulator, [Boolean]) :-
	memberchk(Boolean,[true,false]),
	% Directive is OK
	!.

is_directive_valid(degree_accumulator, [_Boolean]) :-
	% degree_accumulator value is invalid
	!,
	throw_invalid_argument_error(degree_accumulator, 1,
		'degree_accumulator value must be either \'true\' or \'false\'.').

is_directive_valid(degree_accumulator, Arguments) :-
	% Wrong number of arguments
	length(Arguments, Arity),
	throw_wrong_arity_error(degree_accumulator, Arity, 1).

%
% :- ontology_warning(Boolean)
%

is_directive_valid(ontology_warning, [Boolean]) :-
	memberchk(Boolean,[true,false]),
	% Directive is OK
	!.

is_directive_valid(ontology_warning, [_Boolean]) :-
	% Ontology warning value is invalid
	!,
	throw_invalid_argument_error(ontology_warning, 1,
		'Ontology warning value must be either \'true\' or \'false\'.').

is_directive_valid(ontology_warning, Arguments) :-
	% Wrong number of arguments
	length(Arguments, Arity),
	throw_wrong_arity_error(ontology_warning, Arity, 1).
...
is_directive_valid(transitivity, [_Type]) :-
	...
	throw_invalid_argument_error(transitivity, 1,
		'Transitivity type must be in domain [yes, no, min, product, luka, drastic, nilpotent, hamacher].').
...
is_directive_valid(fuzzy_rel, [RelName, Properties]) :-
	...
	(member(transitive(TNorm), Properties) ->
		member(TNorm, [yes, no, min, luka, product, drastic, nilpotent, hamacher])
	;
		true
	),
	% Directive is OK
...
is_directive_valid(fuzzy_rel, [RelName, _Properties]) :-
	...
	throw_invalid_argument_error(fuzzy_rel, 2,
		'"Less/Most general than" relations must be reflexive and transitive, \c
		 and transitive type must be in domain [yes, no, min, product, luka, drastic, nilpotent, hamacher].').
...
is_directive_valid(fuzzy_rel, [RelName, _Properties]) :-
	...
	throw_invalid_argument_error(fuzzy_rel, 2,
		'Closure properties must be a subset of {symmetric, reflexive, \c
		 ...
		 [yes, no, min, product, luka, drastic, nilpotent, hamacher].').
...
is_directive_valid(hypothetical, [Boolean]) :-
	memberchk(Boolean, [true, false]),
	% Directive is OK
	!.

is_directive_valid(hypothetical, [_Boolean]) :-
	% Hypothetical value is invalid
	!,
	throw_invalid_argument_error(hypothetical, 1,
		'Hypothetical value must be either \'true\' or \'false\'.').

is_directive_valid(hypothetical, Arguments) :-
	% Wrong number of arguments
	length(Arguments, Arity),
	throw_wrong_arity_error(hypothetical, Arity, 1).
