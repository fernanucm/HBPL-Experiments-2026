...
run_bousiprolog_test_files([File|MoreFiles], Passed, FinalPassed, Failed, FinalFailed) :-
	...
	(
		...
		run_bousiprolog_tests(Tests, Passed, NewTestsPassed, Failed, NewTestsFailed),
	...
	(
		% Runs each additional query
			...
			forall(member(AdQuery, AdQueries), (
				translator:translate_query(AdQuery, AdExQuery, _NewHypoRules, _Bindings, _Degree),
...
%% display_status/0
%
%     Displays the current status of the most relevant Bousi-Prolog
%     flags.
%

display_status :-
  Flags = [weak_unification, degree_accumulator, hypothetical, indexing, filtering],
  forall(member(FlagName, Flags), 
    (Flag =.. [FlagName, Value],
     flags:get_bpl_flag(Flag),
     format('~w: ~t~20|~w~n', [FlagName, Value]))).
...
bousiprolog_test_files([
    'bpl_relations_1.bpl',        'bpl_relations_2.bpl',
    ...
    'bpl_higherorder_1.bpl',      'bpl_higherorder_2.bpl',
    'bpl_higherorder_3.bpl']).
